...
// Import Component
import SubPopUp1 from '../../components/subPopup1';
import SubPopUp2 from '../../components/subPopUp2';
...
const MymainOther = () => {
    ...
    return (
        <div className='relative w-screen h-screen overflow-hidden'>
            ...
            <div className='fixed top-0 left-[765px] w-[512px] h-[calc(100vh-80px)] bg-[#F9F8F8] shadow-2xl rounded-[30px] p-6 overflow-y-auto no-scrollbar z-10'>
                ...
                {userInfo && (
                    <>
                        <ProfileContainer className='h-[140px] mt-40'>
                            ...
                            <ProfileInfo className='ml-4'>
                                ...
                                <span className='mt-4 flex'>
                                    게시물 {userInfo.recipeCount} · 평균 후기 {userInfo.ratingAverage}{' '}
                                    <FaStar size='16' color='gold' className='ml-2' />
                                </span>
                            </ProfileInfo>
                        </ProfileContainer>
                        ...
                        <PostGrid>
                            {userInfo.recipes &&
                                Array.isArray(userInfo.recipes) &&
            ...
            {/* 구독 취소 버튼 클릭 */}
            {isPopupOpen && !isSubscribed && (
                <SubPopUp1 name={userInfo?.name} id={userInfo?.id} onClose={handleClosePopup} />
            )}
            {/* 구독하기 버튼 클릭 */}
            {isPopupOpen && isSubscribed && (
                <SubPopUp2 name={userInfo?.name} id={userInfo?.id} onClose={handleClosePopup} />
            )}
        </div>
    );
};
