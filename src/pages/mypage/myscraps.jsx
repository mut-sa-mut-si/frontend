...
function MyScraps() {
    ...
    const findScraps = async () => {
        const cleanToken = localStorage.getItem('jwt') ? localStorage.getItem('jwt').replace('Token: ', '') : '';
        try {
            // id값 바꿔야함
            const response = await axios.get(`${api}/api/v1/members/1/scraps`, {
                headers: {
    ...
    return (
        <div className="fixed w-screen h-screen overflow-hidden cursor-pointer">
            <Side />
            <div name="a" className="fixed top-0 left-[765px] w-[512px] h-[calc(100vh-50px)] bg-[#F9F8F8] shadow-2xl rounded-[30px] overflow-y-auto no-scrollbar z-10">
                <div className="sticky fixed flex flex-col items-center top-0 bg-[#F9F8F8] z-20 w-full p-6">
                    ...
                    <div name="b" className="flex items-center mb-6 w-full">
                        ...
                        <h1 className="text-2xl font-bold ml-4">스크랩 레시피</h1>
                    </div>
                </div>
                <div className="p-6">
                    {loading ? (
                        ...
                        recipes.length > 0 ? (
                            <div className="space-y-4">
                                {recipes.map((recipe) => (
                                    <div 
                                        key={recipe.id} 
                                        className="bg-[#E0E8E9] rounded-[20px] p-4 shadow-md cursor-pointer"
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="mr-1">{recipe.member.name}</span>
                                            </div>
                                            <div className="flex items-center space-x-4">
                            ...
                            <div className="flex flex-col items-center justify-center h-full">
                                <p className="text-lg font-bold text-gray-700 mb-4">스크랩한 레시피가 없습니다.</p>
                                <button 
