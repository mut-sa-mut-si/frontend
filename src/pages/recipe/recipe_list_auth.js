...
function RecipeListAuth() {
    ...
    const [selectedRecipe, setSelectedRecipe] = useState(null); // 추가된 부분
    ...
    const handleRecipeClick = (recipe) => {
        if (!recipe.public) {
            setSelectedRecipe(recipe); // 선택된 레시피 설정
    ...
    return (
        <div className='relative w-screen h-screen overflow-hidden'>
            ...
            {isPopupOpen && <PointPopup onClose={closePopup} id={selectedRecipe?.id} />}
        </div>
    );
}
